...
bool stack_push(Stack * ppilha, int dado);//foco
int stack_pop(Stack * ppilha);//foco
int stack_peek(Stack pilha);//foco
void stack_clear(Stack * ppilha);//foco
...
NodeS * NodeS_Index(Stack * ppilha, int indice) {
    for(NodeS * it = ppilha->ptr; it; it = (*it).next) {
...
bool stack_push(Stack * ppilha, int dado){
    //confirmo se pode adicionar com a isfull
    if(!stack_isFull(*ppilha)){
        //malloc
        ...
        //para o caso de estar cheio
        return false;
    }
}

int stack_pop(Stack * ppilha){
    //defino uma variável auxiliar
    NodeS * aux = ppilha->ptr;
    //mudo pra quem a pilha aponta
    ppilha->ptr = aux->next;
    //libero o aux
    free(aux);
    return ppilha->ptr->info;
}  

int stack_peek(Stack pilha){
    //só retorno o topo da pilha
    return pilha.ptr->info;
}

void stack_clear(Stack * ppilha){
    //enquanto tiver coisa
    while (!stack_isEmpty(*ppilha))
    {
        //vou liberando os termos
        NodeS * aux = ppilha->ptr;
        ppilha->ptr = aux->next;
        free(aux);
    }
